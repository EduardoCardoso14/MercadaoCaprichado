...
function TelaEditar2({ navigation, route }) {
  ...
  const fetchProduct = () => {
    ...
    axios.get(`http://172.16.42.89/apiMercadao/select_um/?token=${token}&id=${id}`)
      .then(response => {
  ...
  editar = () => {
    ...
    if (nome2.trim() !== '' && valor2.trim() !== '' && quantidade2.trim() !== '') {
      ...
      axios.post('http://172.16.42.89/apiMercadao/update/', { token, a: id, nome2, valor2, quantidade2, imagem2 })
        .then(response => {
      ...
      <View>
        ...
        <View style={css.principal}>
          ...
          <View style={css.viewletra2}>
            ...
            <Text></Text>
            <View><Button title="Atualizar Produto" color='#154360' onPress={editar} /></View>
          </View>
